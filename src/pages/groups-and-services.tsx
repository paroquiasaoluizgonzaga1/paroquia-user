...
export function GroupsAndServices() {
    ...
    const [groupsAndServices, setGroupsAndServices] = useState<IOtherSchedule | null>(null);
    const [isLoading, setIsLoading] = useState(true);

    useEffect(() => {
        api.get<IOtherSchedule>('other-schedules/first-by-type', {
            params: { type: OtherScheduleTypes.GroupsAndServices },
        })
            .then((response) => {
                setGroupsAndServices(response.data);
            })
            .catch(() => {
                toaster.error({ title: 'Erro ao buscar pastorais, grupos e serviços' });
            })
            .finally(() => {
                setIsLoading(false);
            });
    ...
    return (
        <Flex flexDir={'column'} justify={'center'} align={'center'} gap={8} bg={'#1A2855'} pb={8}>
            ...
            {!isLoading && groupsAndServices && (
                <Stack w="full" px={4} maxW={'800px'} gap={8}>
                    <OtherScheduleCard schedule={groupsAndServices} />
                </Stack>
            )}
            {!isLoading && !groupsAndServices && <EmptyList />}
            <Button
                mt={4}
                rounded={'full'}
                fontSize={'sm'}
